  ...
  return (
    <>
      <div className="main flex flex-col lg:flex-row-reverse items-center lg:justify-between gap-10 lg:gap-20 p-4 lg:p-10 lg:px-20  min-h-screen lg:mb-14">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} className="left w-[60%]">
        ...
        <div className="right w-[100%] lg:w-[100%] h-[100%] flex flex-col justify-center items-start lg:pr-4">
          <motion.h1 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} className='text-[40px] md:text-[50px] font-bold'>Hi, it's <span className='text-[var(--main-color)]'>Danish</span></motion.h1>
          <motion.h2 initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} className='text-[20px] md:text-[25px] font-bold'>I'm a <span className='text-[var(--main-color)]'></span></motion.h2>
          <motion.p initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} className='text-justify sm:text-left '>Welcome to my portfolio! I'm a web developer passionate about creating dynamic, responsive, and user-friendly web applications. With expertise in React, JavaScript, and modern web technologies, I build seamless digital experiences. Explore my projects, skills, and services, and feel free to connect with me for collaboration or inquiries! 🚀</motion.p>
          <div className="social w-full flex gap-6 pt-4 mb-5">
            <motion.a initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} target="_blank" href='https://www.linkedin.com/in/md-danish-63312b309/'><span className='p-4 w-10 h-10 border-2 flex justify-center items-center border-[var(--main-color)] rounded-full text-[var(--main-color)] hover:text-white hover:translate-y-[-5px] hover:scale-110 hover:bg-[var(--main-color)] transition-all'><FontAwesomeIcon color='' size='lg' icon={faLinkedin} /></span></motion.a>
            <motion.a initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} target="_blank" href='https://github.com/Danish0111'><span className='p-4 w-10 h-10 border-2 flex justify-center items-center border-[var(--main-color)] rounded-full text-[var(--main-color)] hover:text-white hover:translate-y-[-5px] hover:scale-110 hover:bg-[var(--main-color)] transition-all'><FontAwesomeIcon color='' size='lg' icon={faGithub} /></span></motion.a>
            <motion.a initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1, delay: 0.5 }} target="_blank" href='https://x.com/MdDanish1664492'><span className='p-4 w-10 h-10 border-2 flex justify-center items-center border-[var(--main-color)] rounded-full text-[var(--main-color)] hover:text-white hover:translate-y-[-5px] hover:scale-110 hover:bg-[var(--main-color)] transition-all'><FontAwesomeIcon color='' size='lg' icon={faTwitter} /></span></motion.a>
          </div>
